...
from fastercache.dsp.parallel_mgr import enable_sequence_parallel, set_parallel_manager, get_sequence_parallel_group
...
def fastercache_STDiT3Block_forward(
        ...
        if counter>7 and counter%2==0:
            x_m = self.cache_attn[1][:x.shape[0]] +(self.cache_attn[1][:x.shape[0]] - self.cache_attn[0][:x.shape[0]]) * (counter - 7)/20.0 * 0.9
        else:
            ...
            if counter == 5:
                self.cache_attn = [x_m, x_m]
            elif counter > 5:
                self.cache_attn[0] = self.cache_attn[1]
                self.cache_attn[1] = x_m
        ...
        if counter>7 and counter%2==0 and 14<= block_id<=42:
            x_m = self.cached_ffn[:x.shape[0]]
        else:
            # modulate (MLP)
            ...
            # MLP
            x_m = self.mlp(x_m)
            if counter>=5 and 14<= block_id<=42:
                self.cached_ffn = x_m
...
if __name__ == "__main__":
    ...
    parser.add_argument("--master_port", default=18189, type=int, help="Master Port")
